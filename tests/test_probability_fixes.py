...
@pytest.mark.parametrize("scenarios, expected_range", [
    (np.random.normal(0.8, 0.01, 10000), (0.001, 0.999)),
])
def test_probability_never_zero_or_hundred_percent(scenarios, expected_range):
    """Test that probabilities are never exactly 0% or 100%"""
    simulator = MonteCarloSimulator(n_simulations=10000)  # Ensure simulator is instantiated
    metrics = simulator.calculate_risk_metrics(scenarios)

    assert expected_range[0] <= metrics['probability_of_loss'] <= expected_range[1], f"Loss probability out of range: {metrics['probability_of_loss']}"
    assert expected_range[0] <= metrics['probability_of_profit'] <= expected_range[1], f"Profit probability out of range: {metrics['probability_of_profit']}"

    print(f"✅ Scenarios: P(Loss)={metrics['probability_of_loss']*100:.2f}%, P(Profit)={metrics['probability_of_profit']*100:.2f}%")


def test_known_normal_distribution():
...
def test_var_probability_consistency():
    ...
    metrics = simulator.calculate_risk_metrics(scenarios_no_edge)
    ...
    metrics = simulator.calculate_risk_metrics(scenarios_suspicious)
...
def test_edge_cases():
    ...
    metrics = simulator.calculate_risk_metrics(scenarios_zero)
    ...
    metrics = simulator.calculate_risk_metrics(scenarios_all_positive)
    ...
    metrics = simulator.calculate_risk_metrics(scenarios_all_negative)
...
def run_probability_tests():
    ...
    # Removed reference to undefined TestProbabilityCalculationFixes class
    
    try:
        test_probability_never_zero_or_hundred_percent()
        test_known_normal_distribution()
        test_var_probability_consistency()
...
def test_probability_with_zero_scenarios(scenarios):
    """Test probability calculation with all zero scenarios"""
    simulator = MonteCarloSimulator(n_simulations=10000)
    metrics = simulator.calculate_risk_metrics(scenarios)
    assert metrics['probability_of_loss'] == 0.001, "Loss probability should be capped at 0.001 for zero scenarios"
    assert metrics['probability_of_profit'] == 0.999, "Profit probability should be capped at 0.999 for zero scenarios"

@pytest.mark.parametrize("scenarios", [
    np.ones(10000),
])
def test_probability_with_all_positive_scenarios(scenarios):
    """Test probability calculation with all positive scenarios"""
    simulator = MonteCarloSimulator(n_simulations=10000)
    metrics = simulator.calculate_risk_metrics(scenarios)
    assert metrics['probability_of_loss'] == 0.001, "Loss probability should be capped at 0.001 for all positive scenarios"
    assert metrics['probability_of_profit'] == 0.999, "Profit probability should be capped at 0.999 for all positive scenarios"

@pytest.mark.parametrize("scenarios", [
    -np.ones(10000),
])
def test_probability_with_all_negative_scenarios(scenarios):
    """Test probability calculation with all negative scenarios"""
    simulator = MonteCarloSimulator(n_simulations=10000)
    metrics = simulator.calculate_risk_metrics(scenarios)
    assert metrics['probability_of_loss'] == 0.999, "Loss probability should be capped at 0.999 for all negative scenarios"
    assert abs(metrics['probability_of_profit'] - 0.001) < 1e-6, "Profit probability should be approximately 0.001 for all negative scenarios"
